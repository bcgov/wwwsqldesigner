...
const setAuthenticationState = (enabled, authenticated, token) => {
  ...
  serverSourceButton.setAttribute(
    "aria-disabled",
    serverAvailable ? "false" : "true",
  );
  if (isAuthenticated && token) {
    document.getElementById("logout-antiforgery-token").value = token;
    globalThis.__wwwSqlCsrfToken = token;
  }
  const settingsLink = document.getElementById("account-settings-link");
  if (settingsLink) settingsLink.hidden = !isAuthenticated;
  if (globalThis.d?.io) {
    globalThis.d.io.setAuthenticationState(isAuthenticated, serverAvailable);
  }
  document.dispatchEvent(new CustomEvent("wwwsql-auth-state"));
};
