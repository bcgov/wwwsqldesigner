...
test("edits, saves, reloads, and safely renders the diagram summary", async ({
  page,
}) => {
  await page.goto("/");
  ...
  expect(
    await page.locator("#maptoggle").evaluate((toggle) => {
  ...
  expect(
    await page.locator("#minimap").evaluate((minimap) => {
      const bounds = minimap.getBoundingClientRect();
      return minimap.contains(
        document.elementFromPoint(bounds.left + 4, bounds.top + 4),
      );
    }),
  ).toBe(true);
  await expect(legend.getByText("Not yet saved", { exact: true })).toHaveCount(
    2,
  );
  await expect(legend.getByLabel("Diagram name")).not.toHaveAttribute(
    "placeholder",
  );
  expect(
    await legend
  ...
  ).toEqual(["Diagram name", "Title", "Application name", "Author"]);
  await page.locator("#maptoggle").click();
  await expect(legend).toBeHidden();
  await page.locator("#maptoggle").click();
  await expect(legend).toBeVisible();
...
test("preserves aspect ratio for tall diagrams in the square minimap", async ({
  page,
}) => {
  await page.goto("/");
...
test("only scrolls the expanded menu when it exceeds the viewport", async ({
  page,
}) => {
  await page.goto("/");
